...
| **No specificity is measured here** | The `controls.cross_task` slot exists and this repository fills it with `[]`. Running the ablation needs a second task in the same harness, which `domains/lm/data/ioi.py` alone does not provide. Until then every circuit emitted here is a within-task claim that says so. |
